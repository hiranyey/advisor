...
class RadarOutput(Base):
    ...
    reason: Mapped[Optional[str]] = mapped_column(String)  # short LLM-written "why call them"
...
class RadarSnapshot(Base):
    """Append-only history of the radar row, one per (client, as_of_date). `radar_output`
    only ever holds the *current* run; this is what lets the dashboard answer "is my book
    getting riskier over time" and "who newly breached since last run" (see book_trend)."""

    __tablename__ = "radar_snapshots"
    __table_args__ = (
        UniqueConstraint("client_id", "as_of_date", name="uq_radar_snapshot_client_date"),
        Index("ix_radar_snapshots_date", "as_of_date"),
        Index("ix_radar_snapshots_client_date", "client_id", "as_of_date"),
    )
    ...
    tolerable_dd: Mapped[Optional[float]] = mapped_column(Numeric)
    simulated_dd: Mapped[Optional[float]] = mapped_column(Numeric)
    flags: Mapped[Optional[list]] = mapped_column(JSONB)
    worst_goal_prob: Mapped[Optional[float]] = mapped_column(Numeric)
    portfolio_value: Mapped[Optional[float]] = mapped_column(Numeric)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now()
    )
...
class BookInsight(Base):
    """Cached LLM-generated book narrative — one batch per as_of_date. The 'briefing' kind
    is the single headline paragraph; the rest are the insight cards the dashboard renders.
    Generated by app.llm.insights.generate_book_insights (nightly + on-demand refresh)."""

    __tablename__ = "book_insights"
    __table_args__ = (
        CheckConstraint(
            "kind in ('briefing','risk','concentration','goals','opportunity')",
            name="book_insights_kind_check",
        ),
        CheckConstraint(
            "severity in ('good','info','watch','critical')",
            name="book_insights_severity_check",
        ),
        Index("ix_book_insights_date", "as_of_date"),
    )
    ...
    client_ids: Mapped[Optional[list]] = mapped_column(JSONB)
